...
module.exports = function(path) {
  ...
  function loadData(page, fn) {
    var dataFiles = page.locals.data.split(',');
    var files = {};
    dataFiles.forEach(function (file) {
      // todo make this name configurable (and remove filename restriction)
      var name = file.split('/').pop().split('.').shift();
      files[name] = function (cb) {
  ...
  function render(page, data, fn) {
    ...
    site.render(page.locals.view, data, function(err, out) {
      if (err) { return fn(err); }
